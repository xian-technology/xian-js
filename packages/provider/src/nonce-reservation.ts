import { ProviderNonceReservationError } from "./errors.js";
import type { XianNumber } from "@xian-tech/types";
...
function toBigInt(value: XianNumber): bigint {
  if (typeof value === "bigint") {
    if (value < 0n) {
      throw new TypeError("network nonce must be a non-negative integer");
    }
    return value;
  }
  if (!Number.isSafeInteger(value) || value < 0) {
    throw new TypeError("network nonce must be a non-negative safe integer");
  }
  return BigInt(value);
}
...
/** Per-provider automatic nonce coordination for intent-based sends. */
    ...
    return this.withState(scope, async (state) => {
      if (state.quarantined !== undefined) {
        ...
        if (networkNonce <= state.quarantined) {
          throw new ProviderNonceReservationError({
            ...scope,
