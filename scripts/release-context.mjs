...
export function validateManifest(manifest, policy) {
  ...
  assertExactKeys(manifest.components, [policy.component], "release manifest components");

  const component = manifest.components[policy.component];
  assertExactKeys(component, ["repository", "ref", "packages"], policy.component);
  if (component.repository !== policy.repository) {
    fail(
      `${policy.component} repository must be ${policy.repository}; got ${component.repository}`,
    );
  }
  if (!SHA_RE.test(component.ref)) {
    fail(`${policy.component} ref must be a lowercase 40-character commit SHA`);
  }

  const expectedPackageNames = Object.keys(policy.componentPackages);
  assertExactKeys(component.packages, expectedPackageNames, `${policy.component} packages`);
  for (const name of expectedPackageNames) {
    const packageSpec = component.packages[name];
    assertExactKeys(packageSpec, ["path", "version"], `${policy.component} package ${name}`);
    const expectedPath = policy.componentPackages[name];
...
export function verifyComponentLock(root, policy, component) {
  const lock = readJson(join(root, "package-lock.json"));
  for (const [name, packageSpec] of Object.entries(component.packages)) {
    const expectedLockPath = `../${policy.component}/${dirname(packageSpec.path)}`;
    const locked = lock.packages?.[expectedLockPath];
    if (!locked) {
      fail(`package-lock.json is missing pinned component path ${expectedLockPath}`);
    }
    if (locked.name !== name || locked.version !== packageSpec.version) {
      fail(
        `package-lock.json ${expectedLockPath} is ${locked.name}@${locked.version}; expected ${name}@${packageSpec.version}`,
      );
    }
  }
}
...
export function resolveReleaseContext(root, { tag, triggerSha, refType }) {
  ...
  const component = validateManifest(manifest, policy);
  const version = releaseVersionFromTag(tag);
  verifySelfVersions(root, policy, version);
  verifyComponentLock(root, policy, component);
  ...
  return { component, policy, sourceSha: head, version };
}

export function verifyPinnedCheckout(root, componentCheckout, { expectedSourceSha, tag }) {
  const policy = policyForRoot(root);
  const component = validateManifest(loadManifest(root), policy);
  const version = releaseVersionFromTag(tag);
  ...
  if (componentHead !== component.ref) {
    fail(`${policy.component} HEAD is ${componentHead}; expected ${component.ref}`);
  }
  assertCleanCheckout(componentCheckout, policy.component);
  for (const [name, packageSpec] of Object.entries(component.packages)) {
    ...
    if (packageJson.name !== name || packageJson.version !== packageSpec.version) {
      fail(
        `${policy.component}/${packageSpec.path} is ${packageJson.name}@${packageJson.version}; expected ${name}@${packageSpec.version}`,
      );
    }
  }
}
...
function writeGithubOutputs(path, context) {
  const prefix = context.policy.component.replaceAll("-", "_");
  appendFileSync(
    path,
    [
      ...
      `${prefix}_ref=${context.component.ref}`,
      "",
    ].join("\n"),
  );
}
...
function main() {
  ...
  if (command === "verify-component") {
    verifyPinnedCheckout(root, requiredEnv("COMPONENT_CHECKOUT"), {
